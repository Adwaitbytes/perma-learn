...
// src/client/ao-mu.js
function deployMessageWith({ fetch: fetch2, MU_URL, logger: _logger }) {
...
function deployProcessWith({ fetch: fetch2, MU_URL, logger: _logger }) {
...
// src/client/ao-cu.js
function loadResultWith({ fetch: fetch2, CU_URL, logger }) {
  return ({ id, processId }) => {
    return of(`${CU_URL}/result/${id}?process-id=${processId}`).map(logger.tap("fetching message result from CU")).chain(fromPromise(
...
function loadTransactionMetaWith({ fetch: fetch2, GATEWAY_URL }) {
  ...
  return (id) => of(id).chain(fromPromise(
    (id2) => fetch2(`${GATEWAY_URL}/graphql`, {
      method: "POST",
...
var createLogger = (name = "@permaweb/ao-sdk") => {
...
function resultWith(env) {
  ...
  return ({ message: message2, process }) => {
    return of({ id: message2, processId: process }).chain(verifyInput).chain(read).map(
      env.logger.tap(
...
function messageWith(env) {
  ...
  return ({ process, data, tags, anchor, signer }) => {
    return of({ id: process, data, tags, anchor, signer }).chain(verifyProcess).chain(uploadMessage).map((ctx) => ctx.messageId).bimap(errFrom, import_ramda7.identity).toPromise();
  };
}
...
// src/index.common.js
var DEFAULT_GATEWAY_URL = globalThis.GATEWAY_URL || "https://arweave.net";
var DEFAULT_MU_URL = globalThis.MU_URL || "https://ao-mu-1.onrender.com";
var DEFAULT_CU_URL = globalThis.CU_URL || "https://ao-cu-1.onrender.com";
function connect({
  ...
  const { locate, validate } = (0, import_ao_scheduler_utils.connect)({ cacheSize: 100, GATEWAY_URL });
  const resultLogger = logger.child("result");
  const result2 = resultWith({
    loadResult: loadResultWith({ fetch, CU_URL, logger: resultLogger }),
    logger: resultLogger
  });
  ...
  const message2 = messageWith({
    ...
    deployMessage: deployMessageWith({ fetch, MU_URL, logger: messageLogger }),
  ...
  const spawn2 = spawnWith({
    ...
    deployProcess: deployProcessWith({ fetch, MU_URL, logger: spawnLogger }),
    logger: spawnLogger
  });
...
// src/index.js
var { result, message, spawn } = connect();
